...
const validatePassword = (password) => {
    ...
    // 3- A a 1
    // 4- A a 1 @
    if (!containsCharacters(password, 1).valid) return containsCharacters(password, 1);
    return validResponse;
};
const validateConfirmPassword = (password, confirmPassword) => {
    if (!validatePassword(password).valid) return setValidationRes(false, 'Password must be valid');
    // If they match
    if (password !== confirmPassword) return setValidationRes(false, 'Passwords must match');
    return validResponse;
};
